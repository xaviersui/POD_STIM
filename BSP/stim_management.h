...
#define CMD_GALV_SEL_NONE                                                          \
  {                                                                                \
    GPIO->P_CLR[CMD_GV_P_PORT].DOUT = ((1 << CMD_GV_P_PIN) | (1 << CMD_GV_N_PIN)); \
    CLR_BRIDGE;                                                                    \
  }
#define CMD_GALV_SEL_NEG                                   \
  {                                                        \
    GPIO->P_SET[CMD_GV_N_PORT].DOUT = (1 << CMD_GV_N_PIN); \
    GPIO->P_SET[CMD_L1_PORT].DOUT = (1 << CMD_L1_PIN);     \
  }

#define CMD_GALV_SEL_POS                                   \
  {                                                        \
    GPIO->P_SET[CMD_GV_P_PORT].DOUT = (1 << CMD_GV_P_PIN); \
...
#define SWITCH_STOP                                                         \
  {                                                                         \
    TIMER_Enable(TIMER_GEN_COURANT, false); /*TIMER_CounterSet(TIMER0,0);*/ \
  }

#define STIM_GEN_RELOAD_NEXT_COUNT(val) Timer_SetMft1Timming(val)
#define STIM_GEN_RESET_COUNT                 \
  {                                          \
    TIMER_TopSet(TIMER_GEN_COURANT, 0xFFFF); \
