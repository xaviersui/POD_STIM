...
void StimManagementHacheurInit(void)
{
  ...
  pStimGenCallback[gStimPatternVeineuxBiphasic_c] = VeineuxBiphase2;
...
StimGenErr_t StimManagementSetDigitalAmplitude(uint16_t amplitude, uint8_t pulseId)
{
  ...
  if (gStimGen_t.stimGenPatternId == gStimPatternVeineuxBiphasic_c)
  {
    ...
    {
      //      if (i > gNbExponentialValue_c / 2 - 1)
      //      {
      //        TablePuissanceCalculee[pulseId][i] = (uint16_t)(digAmpl - (gExponentialValue_percent[i] * digAmpl) / 100); //(fTensionToDac * fGAIN_DAC) + fOFFSET_DAC;
      //      }
      //      else
      //      {
...
void ImpulsBiphasSynchro(void)
{
  ...
  case gBiphaseSynchroState_Sync_c:
  {
    ...
    case 0:
      ...
      else /**< No Pulse */
      {
        if (gStimGen_t.nPulse == 2)
          ...
          STIM_GEN_RELOAD_NEXT_COUNT(gStimGen_t.cntTr - 156 * 20 - 158 * 20);
        i = 0;
        tBiphasState = gBiphasStatePos_c;
      }
      break;
...
void ImpulsMonophas(void)
{
  ...
  case gMonophasStateNeg_c:
    ...
    if (++i < gStimGen_t.nPulse) /**< Next Pulse */
    {
      ...
      tMonophasState = /*gBiphasStatePos_c*/ gMonophasStatePos_c;
    }
    else /**< No Pulse */
    {
      if (gStimGen_t.nPulse == 2)
        STIM_GEN_RELOAD_NEXT_COUNT(gStimGen_t.cntTr - 150 * 20 - 2172);
      else
        STIM_GEN_RELOAD_NEXT_COUNT(gStimGen_t.cntTr - 150 * 20);
      i = 0;
      tMonophasState = gMonophasStatePos_c;
    }
...
void VeineuxBiphas(void)
{
  ...
  case gVeineuxBiphasStatePosRise_c:
    ...
    {
      tVeineuxBiphasState = gVeineuxBiphasStatePosRise_c2; // gVeineuxBiphasStatePosFall_c;
      u8PulseStepIdx = 0;
    }
    /** Sets Next Step Time */
    STIM_GEN_RELOAD_NEXT_COUNT(gStimGen_t.tPulse[i].cntWidth);
...
void VeineuxBiphase2(void)
{
  ...
  case gVeineuxBiphasePosIncr_c:
  {
    ...
    tmp = TablePuissanceCalculee[i][u8PulseStepIdx]; //(uint16_t) TablePuissanceCalculee[i][u8PulseStepIdx];
    DigAmp = NORMAL_MODE | ((tmp & 0x0FFF) << 2);
    SPI_TRANSMIT_DATA(MSB(DigAmp));
    SPI_TRANSMIT_DATA(LSB(DigAmp));
  ...
  {
    tmp = gStimGen_t.tPulse[i].digitalAmplitude - TablePuissanceCalculee[i][u8PulseStepIdx];; //(uint16_t) TablePuissanceCalculee[i][u8PulseStepIdx];
    DigAmp = NORMAL_MODE | ((tmp & 0x0FFF) << 2);
    SPI_TRANSMIT_DATA(MSB(DigAmp));
    SPI_TRANSMIT_DATA(LSB(DigAmp));

    u8PulseStepIdx++;
    if (u8PulseStepIdx == gNbExponentialValue_c - 12)
    {
      gflag[0] = TRUE;
      // STIM_OUT_SEL_NONE;
      u8PulseStepIdx = 0;
...
void Sinus(void)
{
  ...
  case gSinusPos_c:
  {
    ...
    if (u8PulseStepIdx == gNbSinusValue_c / 2)
    {
      STIM_OUT_SEL_NONE; /**< Disables Pulse Output */
      Gpio_SetElectrostimulation(eETAPE4);
      CMD_M_SET_NO_PULSE; /**< Disables pulse CMD */
      gflag[0] = TRUE;
      // Gpio_ClrAop()();
      tSinusMonoState = gSinusNeg_c;
    }
    STIM_GEN_RELOAD_NEXT_COUNT(392 * 20);
    break;
  }
  case gSinusNeg_c:
  {
